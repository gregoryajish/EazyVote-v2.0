...
try:
    ...
    root_err = tk.Tk()
    root_err.withdraw()
    messagebox.showerror("Database Connection Error", f"Failed to connect to the MySQL database:\n{e}\n\nPlease ensure MySQL server is running with the password '1234' and the database 'vimalagiri2026' exists.")
    import sys
    sys.exit(1)

# Verify candidates table has exactly 16 entries
try:
    ...
    root_err = tk.Tk()
    root_err.withdraw()
    messagebox.showerror("Database Query Error", f"Failed to query candidates from the database:\n{e}\n\nPlease ensure tables are created by running 'sqltable.py'.")
    cnx.close()
    import sys
    sys.exit(1)
...
root.state('zoomed')
root.attributes('-fullscreen', True) # Fullscreen kiosk lockdown
...
def execute_vote_update(serial_no):
    ...
    for attempt in range(2):
        try:
            # Check connection, reconnect if dead
            ...
            print(f"Database error on vote update (attempt {attempt+1}): {err}")
...
def verify_unlock(event=None):
    global exit_unlocked
    if x == 8:
        # Student has reached the Congratulations screen.
        # Pressing 'u' or 'U' unlocks the exit button silently (no popup window).
        exit_unlocked = True
